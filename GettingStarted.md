...
`esecui` is a simple IDE for rapid development and testing of evolutionary algorithms  using [ESDL](http://stevedower.id.au/esdl). It provides a code editor, the ability to quickly view relevant results and export to [esec](http://github.com/zooba/esec).

`esecui` is not intended as a statistical workbench or similar. It is (deliberately) not possible to retrieve detailed histories or values from experiments; this level of analysis requires the use of `esec` or another platform supporting ESDL.
...
The ESDL definition is the main algorithm. Everything else is provided to support this definition. For more information on writing an algorithm in ESDL, see [ESDL](http://github.com/zooba/esec/blob/master/ESDL.md), [stevedower.id.au/esdl](http://stevedower.id.au/esdl) and the publications at [stevedower.id.au/publications](http://stevedower.id.au/publications).
...
All species, generators and operators provided by `esec` are available for use in algorithms (see the [Reference Guide](http://github.com/zooba/esec/blob/master/esec%20Quick%20Reference%20(2010-12-17).pdf)), however, the strength of ESDL comes from being able to easily provide customised operators. The Python editor provides this.
...
To report these errors, save your configuration, copy the entire text of the Log tab and attach both to an issue.

# Export

When you have finished designing and testing an algorithm, the Export command (under the Configuration menu, or `Ctrl+E`) will allow you to create a `.py` file suitable for use with `esec`s command line tool. See the `esec` [documentation](http://github.com/zooba/esec) for more details on running a batch file.
